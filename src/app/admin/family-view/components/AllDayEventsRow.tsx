      ...
      member.Events.forEach((event) => {
            ...
            if (!(repeatEnd && selectedDate.isAfter(repeatEnd, "day"))) {
              while (currentStart.isBefore(selectedDate, "day")) {
                switch (rule.Frequency) {
                  case 1:
                    currentStart = currentStart.add(rule.Interval, "day");
                    break;
                  case 2:
                    currentStart = currentStart.add(rule.Interval, "week");
                    break;
                  case 3:
                    currentStart = currentStart.add(rule.Interval, "month");
                    break;
                  case 4:
                    currentStart = currentStart.add(rule.Interval, "year");
                    break;
                  default:
                    currentStart = selectedDate.add(1, "day");
                    break;
                }
              }
        ...
        if (isMatch) {
          ...
          if (!eventMap.has(compositeKey)) {
            ...
            eventMap.set(compositeKey, {
              ...event,
              mergedParticipants: [...(event.EventParticipant || [])],
            });
          } else {
                ...
                {event.EventParticipant?.map((p: any, i: number) => (
                  ...
                  >
                    <Image
                      unoptimized
                      src={
                        memberImageMap[p.ParticipantId] ||
                        memberImageMap[p.id] ||
                        dp.src
                      }
                      alt="participant"
